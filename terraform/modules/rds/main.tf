...
data "aws_secretsmanager_secret" "db_secret" {
  name = "my-db-secret" # fetches the secret metadata using its name from Secrets Manager
}

# Get latest secret value
data "aws_secretsmanager_secret_version" "db_secret_version" {
  secret_id = data.aws_secretsmanager_secret.db_secret.id # uses the secret ID to fetch the latest version of the secret value
}
...
resource "aws_db_instance" "main" {
  ...
  engine_version = "8.0.33"             # compatible MySQL version
